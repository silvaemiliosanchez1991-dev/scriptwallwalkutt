...
function _R15()
	local function r15()
		...
		local animNames = { 
			idle = 	{	
				{ id = "http://www.roblox.com/asset/?id=80103653497738", weight = 1 },
				{ id = "http://www.roblox.com/asset/?id=75794256017298", weight = 1 },
				{ id = "http://www.roblox.com/asset/?id=80103653497738", weight = 9 }
			},
			walk = 	{ 	
				{ id = "http://www.roblox.com/asset/?id=88508412373927", weight = 10 } 
			}, 
			run = 	{
				{ id = "http://www.roblox.com/asset/?id=88508412373927", weight = 10 } 
			}, 
...
-- ============================================================
-- TOGGLE UI: botón draggable para activar/desactivar gravedad
-- ============================================================
...
local holdTime = 0
local HOLD_THRESHOLD = 0.2 -- segundos para considerar "mantener presionado"
...
btn.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or
       input.UserInputType == Enum.UserInputType.Touch then
        ...
        -- Detectar mantener presionado para arrastrar
        spawn(function()
            local startTick = tick()
            while not dragging do
                local held = tick() - startTick
    ...
    if input.UserInputType == Enum.UserInputType.MouseButton1 or
       input.UserInputType == Enum.UserInputType.Touch then
        if not dragging then
            -- Click: toggle gravedad
            gravityEnabled = not gravityEnabled

            if gravityEnabled then
                btn.Text = "On"
                btn.BackgroundColor3 = Color3.fromRGB(50, 180, 80)
                -- Reactivar gravedad custom
                Controller.GetGravityUp = GetGravityUp
            else
                btn.Text = "Off"
                btn.BackgroundColor3 = Color3.fromRGB(200, 50, 50)
                -- Volver a gravedad normal INMEDIATAMENTE
                Controller.GetGravityUp = function(self, oldGravity)
                    return Vector3.new(0, 1, 0)
                end
                -- Forzar la gravedad arriba de vuelta al instante
